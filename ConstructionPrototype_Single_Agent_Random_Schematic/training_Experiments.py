...
env = gym.make("ConstructionPrototypeRandom", render_mode="none", map_size_x=10, map_size_y=10)
...
plt.suptitle(f"Construction(Simplified Average Training Performance ({experiment_repetitions} runs)")
plt.show()

# Save the averages as a csv file
df = pd.DataFrame(experiment_data_averages)
df.to_csv(f"Construction_Simplified_Encoded_Direct_Sampling{experiment_repetitions}repetitions_Averages.csv",
          index=False)
print("Results saved")
