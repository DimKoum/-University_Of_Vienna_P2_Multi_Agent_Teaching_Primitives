...
class Renderer:
    def __init__(self):
        ...
        pygame.display.set_caption('Construction Support')
    ...
    def draw_Map(self, agent_Data, ground_Data):
        ...
        for x_position in list(range(0, self.window_size_X, self.square_Offset_X)):
            for y_position in list(range(0, self.window_size_Y, self.square_Offset_Y)):
                ...
                # Draw terrain
                """ 
                If ground_data x,y value = -1 : Obstacle block  (crimson tile)
                ...
                If ground_data x,y value = 4 : Locked Tile Under Construction (dark blue tile)
                """
                ...
                # Draw agents
                """ 
                If agent_data x,y value = 1 : RepairUnit ( White tile) smaller than a tile
                If agent_data x,y value = 2 : Support unit Agent ( Green Circle) smaller than a tile
                If agent_data x,y value = 3 : RepairUnit receiving maintenance ( Pink tile) smaller than a tile
                """
                if agent_Data[mapX][mapY] == 1:
                    pygame.draw.rect(self.screen, "white",
                                     [x_position + (self.square_Offset_X / 4), y_position + (self.square_Offset_Y / 4),
                                      self.square_Offset_X / 2, self.square_Offset_Y / 2],
                                     0)
                if agent_Data[mapX][mapY] == 2:
                    pygame.draw.rect(self.screen, "green",
                                       [x_position + (self.square_Offset_X / 4),
                                        y_position + (self.square_Offset_Y / 4), self.square_Offset_X / 2,
                                        self.square_Offset_Y / 2],
                                       0)
                if agent_Data[mapX][mapY] == 3:
...
if __name__ == '__main__':
    ...
    while True:
        renderer.render(map_size_X=map_size_X, map_size_Y=map_size_Y, agent_Data=mock_agent_map,
                        ground_Data=mock_ground_map)
